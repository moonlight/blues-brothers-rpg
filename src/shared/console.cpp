...
{
    if (progress > 0) {
        int posY = (int)((double)(dest->h / 4) *
                sin(((0.5 * M_PI) / (double)100) *
                    (double)progress) - text_height(font));

        line(dest, 0, posY + text_height(font) + 1, dest->w - 1,
                posY + text_height(font) + 1, makecol(0,0,0));
        set_trans_blender(0,0,0,100);
        drawing_mode(DRAW_MODE_TRANS, NULL, 0, 0);
        rectfill(dest, 0, 0, dest->w - 1, posY + text_height(font),
                makecol(0,0,0));
...
{
    if ((when & CON_ALWAYS) || ((when & CON_DEBUG) && debug_mode) ||
            ((when & CON_VDEBUG) && debug_mode == 2))
    {
