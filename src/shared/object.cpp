...
void Object::walk(int dir, bool col)
{
    ...
    if (walking == 0) {
        ...
        // WARNING: Still needs collision check!
        if (
                (this->dir == DIR_UP && dir == DIR_DOWN) ||
                (this->dir == DIR_DOWN && dir == DIR_UP) ||
                (this->dir == DIR_LEFT && dir == DIR_RIGHT) ||
                (this->dir == DIR_RIGHT && dir == DIR_LEFT))
        {
            next_x = px;
            next_y = py;
            px = nx;
        ...
        for (i = map->objects.begin(); i != map->objects.end(); i++) {
            ...
            if ((obj->obstacle) &&
                    (((obj->x  + obj->w > next_x && obj->x  <= next_x) &&
                     ...
                     ((obj->nx + obj->w > next_x && obj->nx <= next_x) &&
                      obj->walking > 0 &&
                      (obj->ny - obj->h < next_y && obj->ny >= next_y))))
            {
                callMemberFunction(tableRef, "event_bump_into", "o", obj);
                callMemberFunction(obj->tableRef,
                        "event_bumped_into", "o", this);
                check_stand_on();
                return;
            }
        }
...
void Object::draw(BITMAP *dest, Point screenCoords)
{
    if (bitmap) {
        ...
        if (selected) {
            ...
            if (obstacle) {
                Point p = map->mapToScreen(
                        Point(int(x * TILES_W), int(y * TILES_H)));

                // Draw filled rectangle displaying obstacle region
                set_trans_blender(0,0,0,64);
                drawing_mode(DRAW_MODE_TRANS, NULL, 0, 0);
                rectfill(
                        dest,
                        p.x, p.y,
                        p.x + TILES_W * w, p.y - TILES_H * h,
                        makecol(150,0,0)
                        );
                drawing_mode(DRAW_MODE_SOLID, NULL, 0, 0);
            }
    ...
    if (debug_mode || selected) {
        textprintf_ex(
                dest, font, screenCoords.x - 1, screenCoords.y + 2,
                makecol(128,128,128), -1, "%i, %i", pos.x, pos.y);
    }
}
