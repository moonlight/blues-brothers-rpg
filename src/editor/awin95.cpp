...
int d_awin95_check_proc(int msg, DIALOG *d, int c)
{
	...
	if (d->dp) {
	    int rtm = text_mode(-1);
	    gui_textout(screen, (char *)d->dp, d->x+18, d->y+d->h/2-text_height(font)/2,
...
int d_awin95_edit_proc(int msg, DIALOG *d, int c)
{
	...
	win95_text_bevel(screen, d->x, d->y, d->w, fonth+6);
...
int d_awin95_textbox_proc(int msg, DIALOG *d, int c)
{
    if (msg == MSG_DRAW) {
	int height, bar;
	int fg_color = (d->flags & D_DISABLED) ? gui_mg_color : black;
...
{
    win95_menu_box(screen, x, y, w, h, 0);
}
