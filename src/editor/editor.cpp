...
int menu_item_new_map()
{
    if (close_current_map())
    {
        ...
        if (ret == NEWMAP_OK) {
            ...
            {
                alert(NULL, "Please enter positive values for all variables!",
                        NULL, "OK", NULL, 13, 0);
            }
        }
    }
    ...
    if (close_current_map())
    {
                    ...
                    ))
        {
            ...
            if (doc != NULL) {
                ...
                if (cur == NULL || !xmlStrEqual(cur->name, BAD_CAST "map")) {
                    alert(NULL, "Empty document!", NULL, "OK", NULL, 13, 0);
                    xmlFreeDoc(doc);
                    return D_O_K;
                }

                selectedObjects.clear();
                currentMap->loadFrom(cur, tileRepository);
                xmlFreeDoc(doc);
                ...
                if (file) {
                    ...
                    alert(NULL, "Error while trying to load map!",
                            NULL, "OK", NULL, 13, 0);
                }
            }
        }
...
int menu_item_save_map()
{
    ...
    xmlTextWriterPtr writer = xmlNewTextWriterFilename(map_filename, 0);
    if (writer) {
...
int menu_item_resize_map()
{
    ...
    if (ret == RESIZEMAP_OK)
    {
        ...
        {
            alert(NULL, "Error, illegal value(s) for new map size!",
                    NULL, "OK", NULL, 13, 0);
        }
    }
...
int menu_item_save_map_image()
{
    ...
    if (mapWidth > 0 && mapHeight > 0 && temp_bitmap) {
        int x, y;
...
int menu_item_save_map_as_xml()
{
    ...
    {
        xmlTextWriterPtr writer = xmlNewTextWriterFilename(path_buf, 0);
        if (writer == NULL) {
            alert(NULL, "Error while creating the xml writer!",
                    NULL, "OK", NULL, 13, 0);
        }
        xmlTextWriterSetIndent(writer, 1);
        xmlTextWriterStartDocument(writer, NULL, NULL, NULL);
