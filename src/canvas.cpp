...
void Canvas::init()
{
    curX = 0;
    curY = 0;
    set_clip_rect(buffer, 0, 0, SCREEN_W - 1, SCREEN_H - 1);
    myFont = font;
...
{
    set_clip_rect(buffer, x1, y1, x2, y2);
}
...
{
    ASSERT(text);
    textout_ex(buffer, myFont, text, curX, curY, drawColor, -1);
