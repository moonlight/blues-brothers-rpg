...
{
    module->loadScript(luaL_checkstring(L, 1));
    return 0;
}
    ...
    if (desc)
    {
        while (*desc != '\0')
        {
            switch (*desc) {
                ...
                case 'o': // Object
                    lua_rawgeti(L, LUA_REGISTRYINDEX, (va_arg(vals, Object*))->tableRef); break;
                case 'm': // TiledMap
...
int l_alert(lua_State *L)
{
    ...
    while (lua_getstack(L, level, &ar) != 0)
    {
        ...
        sprintf(error_line,
                ...
                level, ar.name, ar.currentline, ar.short_src
               );
...
int l_get_objects_at(lua_State *L)
{
    ...
    for (i = map->objects.begin(); i != map->objects.end(); i++) {
        ...
        {
            lua_rawgeti(L, LUA_REGISTRYINDEX, obj->tableRef);
            lua_rawseti(L, -2, c);
            c++;
        }
    }
...
int l_import_tile_bmp(lua_State *L)
{
    ...
    console.log(CON_CONSOLE, CON_DEBUG,
            "Importing tile bitmap \"%s\"...", filename);

    BITMAP* tileBitmap = module->findBitmap(filename);

    if (tileBitmap) {
        char tempFilename[256];
        replace_extension(tempFilename, get_filename(filename), "", 256);
        tileRepository->importBitmap(
                tileBitmap, tempFilename, tile_w, tile_h, tile_spacing);
    } else {
        console.log(CON_QUIT, CON_ALWAYS,
                "Error: file for tile bitmap not found: \"%s\"", filename);
    }
...
int l_set_font(lua_State *L)
{
    ...
    FONT* found_font = module->findFont(font_name);

    if (found_font) {
        canvas.setFont(found_font);
    } else {
        console.log(CON_LOG, CON_ALWAYS,
                "Warning: Cannot find requested font! (%s)", font_name);
...
int l_load_map(lua_State *L)
{
    ...
    TiledMap *map = module->loadMap(map_name);
    ...
    if (map) {
        ...
        if (debug_mode) {
            luaL_error(L, "Error: Attempt to load a non-existing map (%s)!",
                    map_name);
        }
        else {
            console.log(CON_LOG | CON_CONSOLE, CON_ALWAYS,
                    "Warning: Attempt to load a non-existing map (%s)!",
                    map_name);
        }
    }
...
int l_get_tile_at(lua_State *L)
{
    ...
    if (map) {
        ...
        console.log(CON_CONSOLE | CON_LOG, CON_DEBUG,
                "Warning: get_tile_at called without a map.");
    }
...
int l_set_tile_at(lua_State *L)
{
    ...
    int x, y;
    char *tileName;
    getLuaArguments(L, "miis", &map, &x, &y, &tileName);
    ...
    if (map) {
        ...
        console.log(CON_CONSOLE | CON_LOG, CON_DEBUG,
                "Warning: set_tile_at called without a map.");
    }
...
int object_settable(lua_State *L)
{
    if (lua_isstring(L, -2))
    {
        ...
        // Exit with error when the object pointer is NULL
        if (!obj) {
            //lua_error(L, "Script tried to set variable of nil object.");
            console.log(CON_LOG, CON_VDEBUG,
                    "Warning: Script tried to set variable of nil object.");
            lua_settop(L, 0);
            return 0;
        }

        if (strcmp(index, "speed") == 0) {
            obj->speed    = lua_tonumber(L, -1);
        }
        else if (strcmp(index, "walking") == 0) {
            obj->walking = lua_tonumber(L, -1);
        }
        else if (strcmp(index, "x") == 0) {
            obj->setX(lua_tonumber(L, -1));
        }
        else if (strcmp(index, "y") == 0) {
            obj->setY(lua_tonumber(L, -1));
        }
        else if (strcmp(index, "w") == 0) {
            obj->w = (int)lua_tonumber(L, -1);
        }
        else if (strcmp(index, "h") == 0) {
            obj->h = (int)lua_tonumber(L, -1);
        }
        else if (strcmp(index, "offset_x") == 0) {
            obj->offset_x = (int)lua_tonumber(L, -1);
        }
        else if (strcmp(index, "offset_y") == 0) {
            obj->offset_y = (int)lua_tonumber(L, -1);
        }
        else if (strcmp(index, "offset_z") == 0) {
            obj->offset_z = (int)lua_tonumber(L, -1);
        }
        else if (strcmp(index, "draw_mode") == 0) {
            obj->drawMode = (int)lua_tonumber(L, -1);
        }
        ...
        else if (strcmp(index, "obstacle") == 0) {
            obj->obstacle = (int)lua_tonumber(L, -1);
        }
        else if (strcmp(index, "in_air") == 0) {
            obj->in_air = (int)lua_tonumber(L, -1);
        }
        else if (strcmp(index, "bitmap") == 0) {
            obj->bitmap = (BITMAP*)lua_touserdata(L, -1);
        }
